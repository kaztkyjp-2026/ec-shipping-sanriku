...
| 楽天市場 | RMS通常購入データCSV → 楽天市場ピッキングリスト（印刷／PDF保存） |
| 楽天市場 | Amazon MCF注文（ひとことメモ CONSUMER-）の自動除外・警告表示 |
| 楽天市場 | 発送完了報告用テンプレート × B2発行済みデータ → 楽天市場発送完了報告CSV生成 |

- 出力ファイルはすべてブラウザのダウンロードとして直接保存（フォルダ選択なし）
- Shift-JIS の入出力に完全対応（クオート内改行を含むRMS CSVにも対応）
- 商品サムネイル（sku-images.json）付きピッキングリスト
